...
$opts = [
    ...
    'zip_name'     => 'website_export_' . date('Y-m-d_His') . '.zip',
        'gpt_mode'   => (int)($_REQUEST['gpt_mode'] ?? 0),       // 1 — включить GPT‑аудит/анти‑дубль в финализаторе
    'job_id'     => preg_replace('~[^A-Za-z0-9_\-]~', '', $_REQUEST['job_id'] ?? ''), // id для лога
...
function getLanguages(PDO $pdo): array {
    // Берём языки ТОЛЬКО из элементов langbadge на сохранённых страницах
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        foreach (($data['elements'] ?? []) as $el) {
            if (($el['type'] ?? '') === 'langbadge' && !empty($el['langs'])) {
                $langs = array_map('trim', explode(',', (string)$el['langs']));
                foreach ($langs as $v) {
                    $v = preg_replace('~[^A-Za-z\-]~', '', $v); // нормализация, поддержка zh-Hans
                    if ($v !== '') $languages[] = $v;
                }
            }
        }
    }
    // Итог — только уникальные и нормализованные коды языков
...
function generatePageHTML($pdo, $page, $lang, $translations, &$usedFiles, $allLanguages, $primaryLang = 'ru', $seoLang = null) {
    // Подключаем SEO Manager + наш мультиязычный хелпер
    require_once __DIR__ . '/../ui/seo/SeoManager.php';
    require_once __DIR__ . '/../ui/seo/MultiLangSeoHelper.php';
    require_once __DIR__ . '/../ui/export-gpt/AltTranslations.php';
...
// Переопределение флагов на время экспорта (если прилетели из формы)
if (isset($_REQUEST['enable_json_ld'])) $seoManager->saveSettings(['enable_json_ld' => (int)$_REQUEST['enable_json_ld'] ? 1 : 0]);
if (isset($_REQUEST['enable_og']))      $seoManager->saveSettings(['enable_og'      => (int)$_REQUEST['enable_og'] ? 1 : 0]);
if (isset($_REQUEST['enable_twitter'])) $seoManager->saveSettings(['enable_twitter' => (int)$_REQUEST['enable_twitter'] ? 1 : 0]);
...
$__withBreadcrumbs = (int)($_REQUEST['breadcrumbs'] ?? 1);
    ...
    // --- AUTO META OVERRIDE из первого HTML-блока (если есть явные <title> / <meta name="description">) ---
try {
    $htmlBlock = '';
    foreach (($data['elements'] ?? []) as $el) {
        if (($el['type'] ?? '') === 'text') {
            $htmlBlock = (string)($el['html'] ?? $el['content'] ?? '');
            if ($htmlBlock !== '') break;
        }
    }
    if ($htmlBlock !== '') {
        // <title>...</title>
        if (preg_match('~<title>(.*?)</title>~is', $htmlBlock, $m) && trim($m[1]) !== '') {
            $title = trim(strip_tags($m[1]));
        }
        // <meta name="description" content="...">
        if (preg_match('~<meta\s+name=["\']description["\']\s+content=["\']([^"\']+)["\']~i', $htmlBlock, $m) && trim($m[1]) !== '') {
            $description = trim($m[1]);
        }
        // og:image из блока — если не задан через настройки
        if (empty($page['og_image']) && preg_match('~<meta\s+property=["\']og:image["\']\s+content=["\']([^"\']+)["\']~i', $htmlBlock, $m)) {
            $page['og_image'] = trim($m[1]);
        }
    }
...
// ---- Stats: используем домен из модалки (если указан) ----
$statsDomainFromForm = trim((string)($_REQUEST['stats_domain'] ?? ''));
$statsDomainHost = $statsDomainFromForm ?: $host;

$statsBase = $statsDomainHost ? "{$scheme}://{$statsDomainHost}" : '';
...
if ($statsToken === '') {
    ...
    // Определяем, сайт вообще мультиязычный или нет
    $allLanguages = is_array($allLanguages) ? array_values(array_unique($allLanguages)) : [];
    $isMultilangSite = count($allLanguages) > 1;

    // Нормализуем основной язык (что ты передаёшь в exportSite)
    $primaryLang = $primaryLang ?: 'ru';

    // Базовый домен для экспорта: из модалки "Домен", либо {{BASE_URL}}
    $exportDomainRaw = trim((string)($_REQUEST['domain'] ?? ''));
    if ($exportDomainRaw !== '' && !preg_match('~^https?://~i', $exportDomainRaw)) {
        $exportDomainRaw = 'https://' . $exportDomainRaw;
    }
    $baseUrlForSeo = $exportDomainRaw !== '' ? rtrim($exportDomainRaw, '/') : '{{BASE_URL}}';

    // По умолчанию делаем красивые URL без .html
    // Если захочешь тумблер в модалке — просто подставишь сюда флаг из $_REQUEST.
    $prettyUrls = true;

    // Базовые SEO‑теги + JSON‑LD от SeoManager (они уже с учётом перевода)
    $seoTags = $seoManager->generateMetaTags($pageForSeo, $htmlLang, $allLanguages);
    $jsonLd  = $seoManager->generateJsonLd($pageForSeo, $htmlLang);

    // Если языков несколько — пересобираем canonical/hreflang/og:url/twitter:url и WebPage в JSON‑LD
    if ($isMultilangSite) {
        // Наш правильный canonical + hreflang под статические файлы
        $canonicalBlock = MultiLangSeoHelper::generateCanonicalAndHreflang(
            $pageForSeo,
            $htmlLang,
            $allLanguages,
            $baseUrlForSeo,
            $primaryLang,
            $prettyUrls,
            $primaryLang           // x-default = основной язык
        );

        // Удаляем canonical/hreflang/og:url/twitter:url, сгенерированные SeoManager
        $seoTags = preg_replace('~<link[^>]+rel=["\']canonical["\'][^>]*>\\s*~i', '', $seoTags);
        $seoTags = preg_replace('~<link[^>]+rel=["\']alternate["\'][^>]*hreflang=["\'][^"\']+["\'][^>]*>\\s*~i', '', $seoTags);
        $seoTags = preg_replace('~<meta[^>]+property=["\']og:url["\'][^>]*>\\s*~i', '', $seoTags);
        $seoTags = preg_replace('~<meta[^>]+name=["\']twitter:url["\'][^>]*>\\s*~i', '', $seoTags);

        // Добавляем наш блок в конец
        $seoTags .= "\n" . $canonicalBlock;

        // Обновляем JSON‑LD: url/name/description/inLanguage для WebPage
        if ($jsonLd !== '' && preg_match('~<script[^>]+application/ld\\+json[^>]*>(.*?)</script>~is', $jsonLd, $m)) {
    $rawJson = trim($m[1]);

    // ВАЖНО: не трогаем $data с элементами страницы, используем отдельную переменную
    $schemas = json_decode($rawJson, true);
    if (json_last_error() === JSON_ERROR_NONE && $schemas !== null) {

        $canonicalUrl = MultiLangSeoHelper::buildPageUrl(
            $baseUrlForSeo,
            $pageForSeo,
            $htmlLang,
            $primaryLang,
        ...
        // Для главной страницы основного языка — URL корня
        if (!empty($pageForSeo['is_home']) && $htmlLang === $primaryLang) {
            $canonicalUrl = rtrim($baseUrlForSeo, '/');
        }

        if (isset($schemas['@type'])) {
            // Одиночный объект
            if (in_array($schemas['@type'] ?? '', ['WebPage', 'WebSite'], true)) {
                $schemas['url']         = $canonicalUrl;
                $schemas['name']        = $pageForSeo['meta_title'] ?? $pageForSeo['name'] ?? '';
                $schemas['description'] = $pageForSeo['meta_description'] ?? '';
                $schemas['inLanguage']  = $htmlLang;
            }
        } elseif (is_array($schemas)) {
            // Массив схем: WebSite, Organization, WebPage
            foreach ($schemas as &$schema) {
                if (!is_array($schema)) continue;
                if (in_array($schema['@type'] ?? '', ['WebPage', 'WebSite'], true)) {
                    $schema['url']         = $canonicalUrl;
                    $schema['name']        = $pageForSeo['meta_title'] ?? $pageForSeo['name'] ?? '';
                    $schema['description'] = $pageForSeo['meta_description'] ?? '';
                    $schema['inLanguage']  = $htmlLang;
                }
            }
            unset($schema);
        }
...
HTML;
    // НОВЫЙ КОД (ПРАВИЛЬНО):
    // Генерируем элементы - разделяем langbadge от остального
    $langbadgeHtml = '';
    foreach ($data['elements'] ?? [] as $element) {
        $elementHtml = generateElement($element, $lang, $translations, $usedFiles, $dataTablet, $dataMobile, $page, $allLanguages, $primaryLang);
        // Если это langbadge, сохраняем отдельно
        if (($element['type'] ?? '') === 'langbadge') {
            $langbadgeHtml .= $elementHtml;
        } else {
            $html .= $elementHtml;
        }
    }
    
  $html .= $langbadgeHtml . <<<HTML
    </div>
        <script>
      (function(){
        var wrap = document.querySelector('.wrap');
        var badge = document.querySelector('.el.langbadge');
        if (wrap && badge && !wrap.contains(badge)) {
...
$textStyle = sprintf(
    ...
    break;
        ...
        case 'langbadge':
            ...
            $style = 'position:absolute!important;' . $style;
            $html = sprintf(
                ...
                $style,
                $id,
                '[]',  // Пустой массив для data-tablet
'[]',  // Пустой массив для data-mobile
                $chipStyle,
                $currentFlag,
...
function processHtmlContent($html, &$usedFiles) {
    ...
    $html = preg_replace_callback(
        ...
        // Локализация alt через GPT (если есть данные для текущего job_id)
    $altTranslations = [];

    // gpt_mode и job_id приходят из модалки «Экспорт GPT»
    $jobId  = isset($_REQUEST['job_id']) ? (string)$_REQUEST['job_id'] : '';
    $gptMode = (int)($_REQUEST['gpt_mode'] ?? 0) === 1;

    if ($gptMode && $jobId !== '') {
        $altTranslations = AltTranslations::loadForJob($jobId);
    }

    if (!empty($altTranslations)) {
        $html = MultiLangSeoHelper::localizeImageAlts($html, $lang, $altTranslations);
    }
...
/* Языковой переключатель (как в редакторе) */
.el.langbadge { position:absolute !important; background: transparent !important; border: none !important; padding: 0 !important; }
